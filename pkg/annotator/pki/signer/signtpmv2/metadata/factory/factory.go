...
func (i *instance) Create(kind string, data json.RawMessage) metadata.Contract {
	if kind != tpmSignerMetadata.Kind {
		return nil
	}
	...
	switch value.Result {
	case tpmSignerMetadata.FailureResult:
		var concrete tpmSignerMetadata.Failure
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	case tpmSignerMetadata.SuccessResult:
		var concrete tpmSignerMetadata.Success
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	}
