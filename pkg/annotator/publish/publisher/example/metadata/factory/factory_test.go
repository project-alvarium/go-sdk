...
func TestInstance_Create(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			test: func(t *testing.T) {
				sut := newSUT()
				value := examplePublisherMetadata.NewFailure(test.FactoryRandomString())

				result := sut.Create(examplePublisherMetadata.Kind, json.RawMessage(testInternal.Marshal(t, value)))

				assert.NotNil(t, result)
				assert.IsType(t, &examplePublisherMetadata.Failure{}, result)
		...
		},
		{
			...
			test: func(t *testing.T) {
				sut := newSUT()
				value := examplePublisherMetadata.NewSuccess()

				result := sut.Create(examplePublisherMetadata.Kind, json.RawMessage(testInternal.Marshal(t, value)))

				assert.NotNil(t, result)
				assert.IsType(t, &examplePublisherMetadata.Success{}, result)
				assert.Equal(t, testInternal.Marshal(t, value), testInternal.Marshal(t, result))
			},
		},
	}
