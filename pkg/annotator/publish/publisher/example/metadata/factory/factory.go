...
func (i *instance) Create(kind string, data json.RawMessage) metadata.Contract {
	if kind != examplePublisherMetadata.Kind {
		return nil
	}
	...
	switch value.Result {
	case examplePublisherMetadata.FailureResult:
		var concrete examplePublisherMetadata.Failure
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	case examplePublisherMetadata.SuccessResult:
		var concrete examplePublisherMetadata.Success
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	}
