...
func (i *instance) Create(kind string, data json.RawMessage) metadata.Contract {
	if kind != ipfsPublisherMetadata.Kind {
		return nil
	}
	...
	switch value.Result {
	case ipfsPublisherMetadata.FailureResult:
		var concrete ipfsPublisherMetadata.Failure
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	case ipfsPublisherMetadata.SuccessResult:
		var concrete ipfsPublisherMetadata.Success
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	}
