...
func TestInstance_Create(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			test: func(t *testing.T) {
				sut := newSUT()
				value := ipfsPublisherMetadata.NewFailure(test.FactoryRandomString())

				result := sut.Create(ipfsPublisherMetadata.Kind, json.RawMessage(testInternal.Marshal(t, value)))

				assert.NotNil(t, result)
				assert.IsType(t, &ipfsPublisherMetadata.Failure{}, result)
		...
		},
		{
			...
			test: func(t *testing.T) {
				sut := newSUT()
				value := ipfsPublisherMetadata.NewSuccess(test.FactoryRandomString())

				result := sut.Create(ipfsPublisherMetadata.Kind, json.RawMessage(testInternal.Marshal(t, value)))

				assert.NotNil(t, result)
				assert.IsType(t, &ipfsPublisherMetadata.Success{}, result)
				assert.Equal(t, testInternal.Marshal(t, value), testInternal.Marshal(t, result))
			},
		},
	}
