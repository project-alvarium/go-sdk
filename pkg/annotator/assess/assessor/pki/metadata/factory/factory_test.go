...
func TestInstance_Create(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			test: func(t *testing.T) {
				sut := newSUT()
				value := pkiAssessorMetadata.NewFailure(test.FactoryRandomString())

				result := sut.Create(pkiAssessorMetadata.Kind, json.RawMessage(testInternal.Marshal(t, value)))

				assert.NotNil(t, result)
				assert.IsType(t, &pkiAssessorMetadata.Failure{}, result)
		...
		},
		{
			...
			test: func(t *testing.T) {
				sut := newSUT()
				value := pkiAssessorMetadata.NewSuccess(true, []string{test.FactoryRandomString()})

				result := sut.Create(pkiAssessorMetadata.Kind, json.RawMessage(testInternal.Marshal(t, value)))

				assert.NotNil(t, result)
				assert.IsType(t, &pkiAssessorMetadata.Success{}, result)
				assert.Equal(t, testInternal.Marshal(t, value), testInternal.Marshal(t, result))
			},
		},
	}
