...
func (i *instance) Create(kind string, data json.RawMessage) metadata.Contract {
	if kind != pkiAssessorMetadata.Kind {
		return nil
	}
	...
	switch value.Result {
	case pkiAssessorMetadata.FailureResult:
		var concrete pkiAssessorMetadata.Failure
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	case pkiAssessorMetadata.SuccessResult:
		var concrete pkiAssessorMetadata.Success
		if err := json.Unmarshal(data, &concrete); err == nil {
			return &concrete
		}
	}
